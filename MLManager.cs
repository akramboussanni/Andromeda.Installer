...
internal static class MLManager
{
    ...
    public static async Task InstallAsync(string gameDir, bool removeUserFiles, MLVersion version, Architecture arch, InstallProgressEventHandler? onProgress, InstallFinishedEventHandler? onFinished)
    {
        ...
        if (AndromedaManager.ShouldInstall(gameDir))
        {
            onProgress?.Invoke(0.95, "Installing Andromeda mod");
            string? andromedaError = await AndromedaManager.InstallAsync(gameDir,
                (progress, status) =>
                {
                    if (status != null)
                    {
                        onProgress?.Invoke(0.95 + progress * 0.05, status);
                    }
                });

            if (andromedaError != null)
            {
                onFinished?.Invoke(andromedaError);
                return;
            }
        }
